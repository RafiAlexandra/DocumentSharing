...
Class Action {
	...
	function delete_file(){
		extract($_POST);
		$doc = $this->db->query("SELECT * FROM ssga where id= $id")->fetch_array();
		$delete = $this->db->query("DELETE FROM ssga where id = ".$id);
		if($delete){
	...
	function save_upload(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO ssga set $data ");
		}else{
			$save = $this->db->query("UPDATE ssga set $data where id = $id");
		}
		if($save){
			return 1;
		}
