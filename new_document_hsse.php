	...
	<div class="card card-outline card-primary">
        ...
        <input type="hidden" name="id" value="<?php echo isset($id) ? $id : '' ?>">
            ...
            <div class="table table-striped files" id="previews">
              ...
              <?php if (isset($file_json) && !empty($file_json)): ?>
    <?php $decoded_json = json_decode($file_json, true); // Decode as an associative array ?>
    <?php if (json_last_error() === JSON_ERROR_NONE && is_array($decoded_json)): ?>
        <?php foreach ($decoded_json as $k => $v): ?>
            <?php if (is_file('assets/uploads/' . $v)): ?>
                <?php $_f = file_get_contents('assets/uploads/' . $v); ?>
                <?php $dname = explode('_', $v); ?>
           <div class="def-item">
            <input type="hidden" class="inp-file" name="fname[]" value="<?php echo $v ?>" data-uuid="<?php echo $k ?>">
              ...
              <?php else: ?>
                File not found: <?= htmlspecialchars($v); ?>
            <?php endif; ?>
        <?php endforeach; ?>
    <?php else: ?>
...
$(function () {
  ...
  var myDropzone = new Dropzone(document.body, { // Make the whole body a dropzone
    url: "ajax_hsse.php?action=upload_file", // Set the url
    thumbnailWidth: 80,
    thumbnailHeight: 80,
    parallelUploads: 20,
    previewTemplate: previewTemplate,
    acceptedFiles:'application/pdf,.doc,.docx,.xlsx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document',
