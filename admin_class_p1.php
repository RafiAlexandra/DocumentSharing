...
Class Action {
	...
	function delete_file(){
		extract($_POST);
		$doc = $this->db->query("SELECT * FROM p1 where id= $id")->fetch_array();
		$delete = $this->db->query("DELETE FROM p1 where id = ".$id);
		if($delete){
	...
	function save_upload(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO p1 set $data ");
		}else{
			$save = $this->db->query("UPDATE p1 set $data where id = $id");
		}
		if($save){
			return 1;
		}
