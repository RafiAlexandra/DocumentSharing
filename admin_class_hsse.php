...
Class Action {
	...
	function delete_file(){
		extract($_POST);
		$doc = $this->db->query("SELECT * FROM hsse where id= $id")->fetch_array();
		$delete = $this->db->query("DELETE FROM hsse where id = ".$id);
		if($delete){
	...
	function save_upload(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO hsse set $data ");
		}else{
			$save = $this->db->query("UPDATE hsse set $data where id = $id");
		}
		if($save){
			return 1;
		}
