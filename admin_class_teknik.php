...
Class Action {
	...
	function delete_file(){
		extract($_POST);
		$doc = $this->db->query("SELECT * FROM teknik where id= $id")->fetch_array();
		$delete = $this->db->query("DELETE FROM teknik where id = ".$id);
		if($delete){
	...
	function save_upload(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO teknik set $data ");
		}else{
			$save = $this->db->query("UPDATE teknik set $data where id = $id");
		}
		if($save){
			return 1;
		}
